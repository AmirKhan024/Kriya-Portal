...
function ScanFlow() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <button onClick={() => router.push(`/clinic/members/${memberId}`)} className="text-slate-400 hover:text-white text-sm mb-4 transition-colors">
        ← Back to member
      </button>
      ...
      {phase === 'done' && final && (
        <div className="mt-6 flex flex-col gap-4">
          ...
          <Button onClick={() => router.push(`/clinic/members/${memberId}`)}>Back to member</Button>
        </div>
      )}
    </div>
  );
