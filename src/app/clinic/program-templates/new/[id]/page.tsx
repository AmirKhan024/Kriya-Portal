...
function EditTemplateInner() {
  ...
  async function loadTemplate() {
    ...
    const res = await apiClient.get<{
      id: string; name: string; segment: 'care' | 'wellness'; status: 'draft' | 'published';
      phases: Array<{
        id: string; name: string; duration_weeks: number; order: number;
        items: Array<{ id: string; game_id: string; game_name: string; category: string; frequency_per_week: number }>;
      }>;
    }>(`/api/v1/program-templates/${templateId}`);
    setLoading(false);
  ...
  async function handleSave(andPublish = false) {
    ...
    setSaving(true);
    const res = await apiClient.patch(`/api/v1/program-templates/${templateId}`, {
    ...
    if (andPublish && currentStatus !== 'published') {
      setPublishing(true);
      ...
      toast({ variant: 'success', title: currentStatus === 'published' ? 'Template updated' : 'Draft saved' });
    }
  }
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        {currentStatus === 'published' && (
          <div className="mb-4 bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 text-sm text-amber-300">
            This template is published. Edits will only affect future member program assignments — existing programs are not modified.
          </div>
        )}
            ...
            <div className="flex-1 bg-white/5 border border-white/10 rounded-xl p-5">
              ...
              {activePhase.items.length === 0 ? (
                <p className="text-slate-500 text-sm py-8 text-center">
                  No exercises yet. All games show as eligible on templates.
                </p>
              ) : (
